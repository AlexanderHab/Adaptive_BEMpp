...
def precMl(x,H_list,I_list,D_list,AzeroInv,steps):
	...
	for i in range(steps-1,-1,-1):
		...
		#x_temp[i] = (H_list[i].transpose()).dot(D_list[i].dot(H_list[i].dot(x)))
...
def inv_power_iteration(A,Pop,max_it):
	...
	while True:
		...
		wv, info = gmres(Pop(A),ev,x0 = ev, tol=1e-12,maxiter=n)	
		#Tracer()()
		#print(info)
...
for theta in theta_vec:
	for lambda_cg in lambda_cg_vec:
		#save name
		save_name = 'screen_cond_artificial_edge'
...
#				error_indicator_mesh = sum(error_indicator)
				...
				###############################################################################
				# do the artificial refinemen towards corner
				###############################################################################


				# target corner
#				target_corner =  np.zeros(3,dtype='float64')
#
#				target_corner[1]= 0.0000
#				target_corner[0] = 1.4142
#
#			    
#
#				elements=[]
...
#					#Tracer()()
#					for j in range(3):
#						if (element.geometry.corners[:,j] == target_corner).all():
#							grid.mark(element)
#							print("element has been marked")
#					
#					#Tracer()()
#
#
...
#
				###############################################################################
				# do the artificial refinemen towards edges
				###############################################################################


				# target edges given through points (cannot be vertical)
				edge_one_p=  np.zeros(3,dtype='float64')
				edge_one_pp=  np.zeros(3,dtype='float64')

				edge_one_p[1] = 1.4142
				edge_one_pp[0] = 1.4142
				
				edge_two_p=  np.zeros(3,dtype='float64')
				edge_two_pp=  np.zeros(3,dtype='float64')

				
				edge_two_p[1] = -1.4142
				edge_two_pp[0] = 1.4142
				
				#compute eq. cor lines

				k_one = (edge_one_p[1] - edge_one_pp[1]) / (edge_one_p[0] - edge_one_pp[0]) 
				k_two =  (edge_two_p[1] - edge_two_pp[1]) / (edge_two_p[0] - edge_two_pp[0]) 

				d_one = edge_one_p[1] - k_one*edge_one_p[0] 
				d_two = edge_two_p[1] - k_one*edge_two_p[0] 
				...
				for i,element in enumerate(grid.leaf_view.entity_iterator(0)):
					...
					#Tracer()()
					for j in range(3):
						#check if point point is on one of the target edges
						#print("check--------------------->",k_one * element.geometry.corners[0,j] + d_one -  element.geometry.corners[1,j])
						#Tracer()()
						if (np.abs(k_one * element.geometry.corners[0,j] + d_one -  element.geometry.corners[1,j])<=10**(-12)): 
							grid.mark(element)
						#print("element has been marked")
					
						if (np.abs(k_two * element.geometry.corners[0,j] + d_two -  element.geometry.corners[1,j])<=10**(-12)): 
							grid.mark(element)
							#print("element has benn marked")
					#Tracer()()
				...
				vec,max_ev_non_prec = power_iteration(slp_mat,Id_op_cond,30)
				vec,min_ev_non_prec = inv_power_iteration(slp_mat,Id_op_cond,30)
				...
				vec,max_ev_prec = power_iteration(slp_mat,Pinv,30)
				vec,min_ev_prec = inv_power_iteration(slp_mat,Pinv,30)
				...
				vec,max_ev_diagprec = power_iteration(slp_mat,Diag_op_cond,30)
				vec,min_ev_diagprec = inv_power_iteration(slp_mat,Diag_op_cond,30)
