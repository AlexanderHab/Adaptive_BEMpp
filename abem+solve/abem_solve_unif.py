...
#pcg parameter
lambda_cg_vec = [1,0.1,0.01,0.001,0.0001]
	...
	for lambda_cg in lambda_cg_vec:
		...
		while (grid.leaf_view.entity_count(0) <= max_el):
			...
			#for step_counter in range(ada_steps):
				...
				#DEBUG ############### use solve
				phi_exact_coeff = np.linalg.solve(slp_mat, mass_mat.dot(rhs_fun_cg.coefficients))
