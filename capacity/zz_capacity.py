#########################################################
#########################################################
# computes the capacity of a given polyhedra
#
#
#
#
...
grid = bempp.api.import_grid('cube_zz_est_theta0.4_29+10.msh')
...
# file name
save_name = 'cube_zz_est_theta' + str(theta) +'_29+10+'
...
for step_counter in range(steps):
	...
	print("#########################################################################")
	print("ZZ-estimator for capacity cube, p0")
	print("#########################################################################")
	print("adaptive step", step_counter+1)

	print("building up operators")
	...
	bary_index_set = bary_grid.leaf_view.index_set()
       	...
       	############################################################
       	# compute the ZZ error estimator eta = ||h^{1/2} (phi_\ell - I_\ell \phi_\ell)  ||_{L^2(T)) 
	
	# interpret the value of \phi_\ell \in P^0(\TT^\dual) as node value of $I_\ell \phi_\ell \in S^1(\TT_\ell)
	############################################################
...
#		grid.mark(element) # mark for uniform refinement 	# uniform refinement
	...
	error_indicator_mesh = sum(local_est)
	...
	fname = save_name + 'res_plot' +str(step_counter)+'.msh'

	bempp.api.export(grid_function=final_res ,file_name=fname)


	############################################################
	# save grid

	fname = save_name + str(step_counter) + '.msh'
	...
	with open(fname,'wb') as outfile:
		...
		outfile.write(bytes('# mini cube hmat 32k 2000 e-12 solve e-12 q20  \n','UTF-8'))
		outfile.write(bytes('###################################### \n','UTF-8'))
