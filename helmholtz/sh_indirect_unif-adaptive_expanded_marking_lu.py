...
save_name = 'diss_helmholtz_sh_Lshape_p1_comptheta02_expandedmarking_lu'
theta=0.2
...
for k in kvec:
	...
	def neumann_data(x,n,domain_index,result):
		result[0] = 1j*k*(a_x*n[0] + a_y*n[1] + a_z*n[2]) * np.exp(1j*k* (a_x*x[0] + a_y*x[1] + a_z*x[2]))
...
#			( (x[0]-ps[0])**2 +(x[1]-ps[1])**2 +(x[2]-ps[2])**2 )**(-1./2)
	...
	for step_counter in range(unif_steps):
		...
		p0_space = bempp.api.function_space(grid, "P", p)
		space = bempp.api.function_space(grid, "P", p+1)
		#osc_space = bempp.api.function_space(grid, "P", p+5)
    
		neumann_fun = bempp.api.GridFunction(p0_space, fun=neumann_data)
		...
		hlp = bempp.api.operators.boundary.helmholtz.hypersingular(
		...
		#phi,info =  bempp.api.linalg.cg(hlp, rhs, tol=1E-4, callback = callback)
		#print("Number of cg - iterations: {0}".format(number_of_iterations))


		############### use lu
#		u = bempp.api.linalg.lu(hlp, rhs)
		...
		discrete_op = hlp.strong_form()
	
		sol_vec, info = gmres(discrete_op, rhs.coefficients,tol=1e-6,callback=callback)
		print("Number of gmres - iterations: {0}".format(number_of_iterations))

		u = bempp.api.GridFunction(p0_space, coefficients=sol_vec)	#31.1
		...
		my_hlo_op = bempp.api.operators.boundary.helmholtz.hypersingular(
			p0_space, space, space,k)

		my_id_op = bempp.api.operators.boundary.sparse.identity(
			space, space, space)

		neumann_fun_res = bempp.api.GridFunction(space, fun=neumann_data)

	
		residual = my_id_op*neumann_fun_res - my_hlo_op*u
		...
		for i,element in enumerate(grid.leaf_view.entity_iterator(0)):
			...
			error_estimator[i] = element.geometry.volume**(1./2)*residual.l2_norm(element)**2
		...
		unif_iteration_steps[step_counter] = 0 #number_of_iterations
...
#		u = bempp.api.GridFunction(p0_space, coefficients=sol_vec)	#31.1
		...
		error_estimator = np.zeros(grid.leaf_view.entity_count(0),dtype='float64')
		local_mesh_size = np.zeros(grid.leaf_view.entity_count(0),dtype='float64')
		...
		for i,element in enumerate(grid.leaf_view.entity_iterator(0)):
			...
			# compute error indicators
			error_estimator[i] = element.geometry.volume**(1./2)*residual.l2_norm(element)**2
			local_mesh_size[i]= element.geometry.volume
		...
		# get the indices of the sortet array (descending)
		idx = np.argsort((-1)*local_mesh_size)


		# do the expanded marking
		...
		ada_iteration_steps[step_counter] = 0 #number_of_iterations
